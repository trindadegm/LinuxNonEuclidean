...
Engine::Engine() : window(NULL), glContext(NULL) { //hWnd(NULL), hDC(NULL), hRC(NULL) {
  //SetProcessDPIAware();
  CreateGLWindow();
  InitGLObjects();
  //SetupInputs();
...
int Engine::Run() {
  ...
  SDL_Event event;
  while (!quit) { //(true) {
    while (SDL_PollEvent(&event) != 0) { // We have events
...
void Engine::CreateGLWindow() {
  ...
  // In this mode, cursor is hidden and position is not set, but relative
  // movement events are still captured.
  ...
  window = SDL_CreateWindow(GH_TITLE,
...
void Engine::InitGLObjects() {
  ...
  if (SDL_GL_SetSwapInterval(1) < 0) {
    std::cerr << "Unable to set VSYNC... oh well\n";
  }
}
...
void Engine::ToggleFullscreen() {
  ...
  if (isFullscreen) {
    std::cout << "Should be going fullscreen\n";
    ...
    std::cout << "Should be going windowed\n";
    //iWidth = GH_SCREEN_WIDTH;
    //iHeight = GH_SCREEN_HEIGHT;
